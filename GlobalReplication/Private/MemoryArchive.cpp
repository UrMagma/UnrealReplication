...
// We need to access the private 'Data' member of FString, so we define its Serialize function here.
// This requires a friend declaration in FString, which we've removed for a public Serialize method.
// Let's get the raw data via a public method instead.
#include <string>
...
void FString::Serialize(FArchive& Ar)
{
    ...
    else
    {
        int32_t Size = 0;
        Ar << Size;
        // This is tricky. We can't easily get a mutable buffer from std::string without shenanigans.
        // Let's create a temporary buffer.
        std::vector<char> TempBuffer(Size);
        if (Size > 0)
        {
            Ar.Serialize(TempBuffer.data(), Size);
        }
        *this = FString(std::string(TempBuffer.begin(), TempBuffer.end()));
    }
}
